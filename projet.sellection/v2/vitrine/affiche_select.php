...
<?php
    ...
    if ($mysqli->connect_errno) {
        $error_message = "Problème de connexion à la base de données. (Errno: " . $mysqli->connect_errno . ")";
    }

    if (!$error_message && !$mysqli->set_charset("utf8")) {
        $error_message = "Problème de chargement du jeu de caractères UTF-8 : " . $mysqli->error;
    }
    ...
    if (!$error_message) {
        ...
        // Élément suivant
        $req_next = "SELECT elt_numero, elt_titre, elt_descriptif, elt_date_publication, elt_fichier_image, elt_etat, lie_numero, lie_titre FROM t_rassemble_ras LEFT JOIN t_element_elt USING(elt_numero) LEFT JOIN t_selection_sel USING(sel_numero) LEFT JOIN t_lien_lie USING(elt_numero) WHERE elt_numero > $eid AND sel_numero = $sid ORDER BY elt_numero ASC LIMIT 1;";
                    ...
                    <div class="col-md-6">
                        <div class="card shadow">
                            <div class="card-body text-center">
                                ...
                                <a href="select.php" class="btn btn-outline-secondary">Retour à la page selection</a>
                            </div>
                        </div>
                    </div>
                </div>
                ...
                <div class="row">
                    <div class="col-12">
                        <table class="table table-hover table-bordered">
                            ...
                            <tbody>
                                <tr>
                                    <td><img width="100" src="../ressources/<?php echo htmlspecialchars($elt_id['elt_fichier_image']); ?>" alt="<?php echo htmlspecialchars($elt_id['elt_titre']); ?>"></td>
                                    <td><?php echo htmlspecialchars($elt_id['elt_titre']); ?></td>
                                    <td><?php echo htmlspecialchars($elt_id['elt_descriptif']); ?></td>
                                    <td><?php echo htmlspecialchars($elt_id['elt_date_publication']); ?></td>
                                    <td><?php echo htmlspecialchars($elt_id['elt_etat']); ?></td>
                                </tr>
                            </tbody>
                        </table>
                        ...
                        <?php if ($res_next->num_rows == 1): ?>
                            Lien vers élément suivant :
                            <a href="affiche_select.php?sel_numero=<?php echo $sid; ?>&elt_numero=<?php echo $next['elt_numero']; ?>" class="btn btn-link">Suivant</a>
                        <?php else: ?>
                            <p class="text-muted">Il n'y a pas d'élément suivant.</p>
                        <?php endif; ?>
                    </div>
                </div>
            <?php endif; ?>
